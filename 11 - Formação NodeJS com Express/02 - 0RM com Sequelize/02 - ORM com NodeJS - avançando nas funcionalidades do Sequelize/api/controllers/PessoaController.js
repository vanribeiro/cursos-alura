...
class PessoaController {
	...
	static async cadastrarPessoa(req, res) {
		...
		try {
			const pessoa = await pessoasServices.criaRegistro(corpoDaRequisicao);
			return res.status(200).json(pessoa);
		} catch (error) {
			return res.status(500).json(error.message);
		}
	...
	static async atualizarPessoa(req, res) {
		...
		const { id } = req.params;
		try {
			await pessoasServices.atualizaRegistro(corpoDaRequisicao, Number(id));
	...
	static async cancelaPessoa(req, res) {
		...
		try {
			...
			return res.status(200).json({message: `matrículas ref. ao estudante id ${estudanteId} canceladas`});
		} catch (error) {
			return res.status(500).json(error.message);
		}
	}
